...
exports.createLead = (req, res) => {
  ...
  LeadModel.createCustomer(name, email, phone, (err, result) => {
    if (err) return res.status(500).json({ error: "Customer creation failed" });

    const customerId = result.insertId;

    LeadModel.createLead(customerId, status, source, assigned_to, interested_in, (leadErr) => {
      if (leadErr) return res.status(500).json({ error: "Lead creation failed" });
...
exports.getMyLeads = (req, res) => {
  ...
  LeadModel.getLeadsByUser(userId, (err, results) => {
...
exports.updateLead = (req, res) => {
  ...
  LeadModel.updateLead(leadId, status, notes, (err) => {
    if (err) return res.status(500).json({ error: "Lead update failed" });
